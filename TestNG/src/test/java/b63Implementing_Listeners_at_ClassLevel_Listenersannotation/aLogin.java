package b63Implementing_Listeners_at_ClassLevel_Listenersannotation;
...
/*
 * In previous session we have learned how to create MyTestListeners by go to 
   source and click on Override unimplemented methods.
   
 * This session is continuation of previous session in previous session we have
   learned how to create MyTestListeners and that MyTestListeners is how to link
   to aLogin class by @Listeners(MyTestListeners.class) MyTestListeners is class name.
   In this we learn only how to link that MyTestListeners to only one class.
   
 * In next session we learn how to link that MyTestListeners at suite level, suite
   level means under suite there are so many classes, we can link that MyTestListeners
   to multiple classes, because writing @Listeners(MyTestListeners.class) above each 
   and every class is not a good practice.
 */
